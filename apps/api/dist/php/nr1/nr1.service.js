...
exports.Nr1Service = void 0;
const common_1 = require("@nestjs/common");
const config_1 = require("@nestjs/config");
const supabase_js_1 = require("@supabase/supabase-js");
let Nr1Service = class Nr1Service {
    ...
    async createAssessment(dto, assessedBy) {
    ...
    async getComplianceReport(orgId) {
        ...
        const highRiskCount = assessments.filter((a) => a.overall_risk_level === 'high').length;
        const mediumRiskCount = assessments.filter((a) => a.overall_risk_level === 'medium').length;
        const lowRiskCount = assessments.filter((a) => a.overall_risk_level === 'low').length;
        ...
        return {
            ...
            summary: {
                ...
                compliance_status: highRiskCount === 0 && mediumRiskCount < totalAssessments * 0.3
                    ? 'compliant'
                    : 'requires_action',
            },
    ...
    async generateActionPlans(orgId, minRiskLevel, createdBy) {
        const { data: assessments, error } = await this.supabase
            .from('nr1_risk_assessments')
            .select('*')
            .eq('org_id', orgId)
        ...
        for (const assessment of assessments) {
            const highRiskDimensions = this.getHighRiskDimensions(assessment, minRiskLevel);
            if (highRiskDimensions.length > 0) {
                const { data: plan, error: planError } = await this.supabase
                    ...
                    root_cause: highRiskDimensions
                        ...
                        .join(', '),
                    recommended_actions: this.getRecommendedActions(highRiskDimensions),
    ...
    calculateOverallRisk(data) {
        if (data.length === 0)
            return 'low';
        const avgHighRisk = data.reduce((sum, row) => sum + (row.high_risk_count || 0), 0) /
            data.length;
        if (avgHighRisk >= 2)
            return 'high';
        if (avgHighRisk >= 1)
    ...
    calculateDimensionAverages(assessments) {
        const dimensions = [
            'workload_pace_risk',
            'goal_pressure_risk',
            'role_clarity_risk',
    ...
    getHighRiskDimensions(assessment, minRiskLevel) {
        const dimensions = [
    ...
    getRecommendedActions(dimensions) {
    ...
    getActionsForDimension(dimension) {
        const actionMap = {
    ...
    generateRecommendations(criticalDimensions) {
        ...
        if (criticalDimensions.length >= 5) {
            recommendations.push('Considerar consultoria externa especializada em saúde ocupacional');
        }
        return recommendations;
    }
    async createSelfAssessment(dto, userId) {
        const { data, error } = await this.supabase
    ...
    async getSelfAssessment(id) {
      ...
      `)
            ...
            .single();
        if (error)
            throw new common_1.NotFoundException('Self-assessment not found');
        return data;
    }
    ...
    generateComparativeInsights(stats) {
        const insights = [];
        if (stats.critical_gaps > 0) {
            insights.push(`⚠️ ${stats.critical_gaps} funcionário(s) com percepção MUITO diferente da avaliação organizacional - requer atenção urgente`);
        }
        if (stats.optimistic_bias_count > stats.pessimistic_bias_count) {
            insights.push(`🔍 Funcionários tendem a avaliar o ambiente MELHOR do que a organização - pode indicar falta de conscientização sobre riscos reais`);
        }
        else if (stats.pessimistic_bias_count > stats.optimistic_bias_count) {
            insights.push(`🔍 Funcionários tendem a avaliar o ambiente PIOR do que a organização - pode indicar blind spots da gestão`);
        }
        if (stats.aligned > stats.total_comparisons * 0.7) {
            insights.push(`✅ Boa alinhamento entre percepção dos funcionários e avaliação organizacional (${Math.round((stats.aligned / stats.total_comparisons) * 100)}%)`);
        }
        if (Math.abs(parseFloat(stats.average_perception_gap)) > 1.0) {
            insights.push(`📊 Gap médio de percepção significativo (${stats.average_perception_gap}) - revisar comunicação e transparência`);
        }
        return insights.length > 0 ? insights : ['Dados insuficientes para gerar insights'];
    }
    async createInvitations(data, invitedBy) {
        const invitations = [];
        for (const employee_id of data.employee_ids) {
            ...
            if (error) {
                if (error.code === '23505') {
                    throw new Error(`Funcionário já possui convite ativo. Cancele ou aguarde expiração.`);
                }
    ...
    async resendInvitation(id) {
        const newToken = Buffer.from(Array.from({ length: 32 }, () => Math.floor(Math.random() * 256))).toString('base64');
        const { data, error } = await this.supabase
            .from('nr1_assessment_invitations')
            .update({
            token: newToken,
    ...
    async linkInvitationToSelfAssessment(invitationId, selfAssessmentId) {
        const { error } = await this.supabase
            .from('nr1_assessment_invitations')
            .update({
            status: 'completed',
            ...
            .eq('id', invitationId);
        if (error) {
            throw new Error(`Erro ao vincular convite à avaliação: ${error.message}`);
        }
    }
};
exports.Nr1Service = Nr1Service;
exports.Nr1Service = Nr1Service = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [config_1.ConfigService])
], Nr1Service);
//# sourceMappingURL=nr1.service.js.map