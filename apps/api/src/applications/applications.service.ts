...
export class ApplicationsService {
  ...
  async create(dto: CreateApplicationDto, orgId: string, userId: string) {
  ...
  async findAll(
    orgId: string,
    query: { jobId?: string; status?: string; stageId?: string },
  ) {
    const supabase = this.supabaseService.getAdminClient();
    ...
    return data.map((app: any) => this.mapToResponse(app));
  }
  ...
  async getKanbanBoard(jobId: string, orgId: string) {
    const supabase = this.supabaseService.getAdminClient();
    ...
    // Group by stage
    const stages = (job.pipeline_stages as any[])
