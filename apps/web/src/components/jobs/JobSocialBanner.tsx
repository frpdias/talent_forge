...
export interface JobBannerData {
  ...
  seniority: string | null;       // jobs.seniority
  salary_range: string | null;    // jobs.salary_range (text)
  salary_min: number | null;      // jobs.salary_min (fallback)
  salary_max: number | null;      // jobs.salary_max (fallback)
}
...
function SocialIcon({ path, color, url }: { path: string; color: string; url: string }) {
  return (
    <div
      data-pdf-link={url}
      style={{
...
export const JobSocialBanner = forwardRef<HTMLDivElement, { job: JobBannerData; org: OrgBannerData }>(
  ({ job, org }, ref) => {
    ...
    // salary: prefer text, fallback to numbers
    if (job.salary_range) {
    ...
    const titleSize = titleLen > 40 ? 56 : titleLen > 28 ? 68 : 82;
    ...
    return (
      ...
      >
        ...
        {/* ── Gradient overlay — light top, very dark bottom ── */}
        <div style={{
          position: 'absolute', inset: 0,
          background: 'linear-gradient(to bottom, rgba(0,0,0,0.25) 0%, rgba(0,0,0,0.5) 40%, rgba(0,0,0,0.92) 75%, rgba(0,0,0,0.97) 100%)',
        }} />
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
            {logoUrl ? (
              ...
              <div style={{
                width: 90, height: 90, borderRadius: 20, backgroundColor: secondary,
            ...
            <div>
              <div style={{
                color: secondary, fontSize: 14, fontWeight: 800,
                letterSpacing: 5, textTransform: 'uppercase', marginBottom: 6,
              }}>
                ESTAMOS CONTRATANDO
              </div>
              <div style={{ color: '#fff', fontSize: 26, fontWeight: 700, letterSpacing: 0.5 }}>
                {org.name}
              </div>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
              {socials.length > 0
                ? socials.map((s, i) => (
                    <SocialIcon key={i} path={s.path} color={s.color} url={s.url} />
                  ))
                : (
                  <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 18 }}>
                    {org.name}
                  </div>
                )
              }
...
// ─── PDF Download — links calculados a partir das posições reais no DOM ───────
...
export async function downloadJobBannerPDF(
  ...
  // Calcular posições dos links a partir do DOM real (evita hardcode)
  const bannerRect = element.getBoundingClientRect();
  const linkEls = element.querySelectorAll('[data-pdf-link]');
