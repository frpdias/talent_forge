...
export default function ColorTestPage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (qErr) throw qErr;
        const mapped: Question[] = (qData ?? []).map((q: any) => ({
          id: q.id,
          prompt: q.prompt,
          options: [
  ...
  const handleSelect = async (color: ColorCode) => {
    if (!currentQuestion || !assessmentId || saving) return;
    setSaving(true);
    ...
    try {
      const { error: rErr } = await supabase
        ...
        .upsert(
          { assessment_id: assessmentId, question_id: currentQuestion.id, selected_color: color },
  ...
  const finalizeTest = async (finalAnswers?: Record<string, ColorCode>) => {
    ...
    try {
      ...
      Object.values(answersToUse).forEach((c) => { scores[c] = (scores[c] ?? 0) + 1; });
      const order = (Object.keys(scores) as ColorCode[]).sort((a, b) => scores[b] - scores[a]);
      const primary = order[0];
      const secondary = order[1];
      const { error: uErr } = await supabase
        .from('color_assessments')
        .update({ status: 'completed', completed_at: new Date().toISOString(), primary_color: primary, secondary_color: secondary, scores })
        .eq('id', assessmentId);
      if (uErr) throw uErr;
      setResult({ scores, primary, secondary, order });
    } catch (err: any) {
